...
function Pointer() {
  ...
  /**
   * The JSOIN Pointer path within the file, starting with a hash ("#") character.
   *
...
/**
 * Returns a human-friendly representation of the Pointer object.
 *
 * @returns {string}
 */
 ...
 *
 * @param {string} url - An absolute URL with the JSON Pointer path in the hash.
...
Pointer.resolve = function resolve(url, schema, options) {
  ...
  pointer.file = FileArray.get(schema.files, URL.stripHash(url));
  pointer.path = URL.getHash(url);
  pointer.value = pointer.file.data;
