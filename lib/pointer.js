...
function Pointer($ref, url) {
  ...
  /**
   * The URL, containing the JSON pointer in the hash.
   * This URL is relative to the URL of the main JSON schema file.
   * @type {string}
   */
  this.url = url;
 ...
 * the {@link Pointer#$ref} and {@link Pointer#url} will reflect the resolution path
...
Pointer.prototype.resolve = function(obj, options) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    if (resolveIf$Ref(this, options)) {
      // The $ref url has changed, so append the remaining tokens to the url
      this.url = Pointer.join(this.url, tokens.slice(i));
    }

    var token = tokens[i];
    if (this.value[token] === undefined) {
      throw ono.syntax('Error resolving $ref pointer "%s". \nToken "%s" does not exist.', this.url, token);
    }
    else {
 ...
 * @returns {string}
 */
Pointer.join = function(url, tokens) {
 ...
 * In addition, {@link Pointer#url} and {@link Pointer#$ref} are updated to reflect the
...
function resolveIf$Ref(pointer, options) {
  ...
  if ($Ref.isAllowed$Ref(pointer.value, options)) {
    var $refPath = URL.resolve(pointer.url, pointer.value.$ref);

    if ($refPath === pointer.url) {
