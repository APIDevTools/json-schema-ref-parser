...
Pointer.prototype.resolve = function (obj, options, pathFromRoot) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (this.value[token] === undefined || this.value[token] === null) {
      ...
      let targetRef = this.path.replace(this.$ref.path, '')
      let targetFound = Pointer.join('', found)
      let parentPath = pathFromRoot.replace(this.$ref.path, '')

      throw new MissingPointerError(token, this.originalPath, targetRef, targetFound, parentPath);
    }
    else {
