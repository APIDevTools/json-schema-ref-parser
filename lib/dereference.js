    ...
    if ($Ref.isAllowed$Ref(obj, options)) {
      ...
      Object.keys(obj).forEach(function(key) {
        ...
        if ($Ref.isAllowed$Ref(value, options)) {
          ...
          // For example, in the case of anyOf nodes, we may want to dereference the references later,
          // as this will allow us to use the reference name, such as when we write
          // a resolve function for GraphQL and we then use the reference name for the type name.
          // If we dereference the references now, we lose the reference names.
          var ignoreNode = false;
          ignoreNode = options.dereference.ignoreNodes && options.dereference.ignoreNodes.reduce(function(previous, current) {
