...
function resolve$Ref($ref, url, schema, options) {
  ...
  // Ignore any JSON Pointer hash for now. We only care about the file path.
  var fileUrl = URL.stripHash($ref.$ref);

  // Resolve the file path against the current file
  fileUrl = URL.resolve(url, fileUrl);

  if (FileArray.exists(schema.files, fileUrl)) {
