...
function remap(inventory) {
  ...
  var item, file, hash, pathFromRoot;
  for (var i = 0; i < inventory.length; i++) {
    item = inventory[i];
    debug('Re-mapping $ref pointer "%s" at %s', item.$ref.$ref, item.pathFromRoot);

    if (!item.external) {
      // This $ref already resolves to the main JSON Schema file
      item.$ref.$ref = item.hash;
    }
    else if (item.file === file && item.hash === hash) {
      // This $ref points to the same value as the prevous $ref, so remap it to the same path
      item.$ref.$ref = pathFromRoot;
    }
    else if (item.file === file && item.hash.indexOf(hash + '/') === 0) {
      // This $ref points to the a sub-value as the prevous $ref, so remap it beneath that path
      item.$ref.$ref = Pointer.join(pathFromRoot, Pointer.parse(item.hash));
    }
    else {
      ...
      item.$ref = item.parent[item.key] = $Ref.dereference(item.$ref, item.value);

      if (item.circular) {
        // This $ref points to itself
        item.$ref.$ref = item.pathFromRoot;
      }
    }

    debug('    new value: %s', (item.$ref && item.$ref.$ref) ? item.$ref.$ref : '[object Object]');
  }
}
