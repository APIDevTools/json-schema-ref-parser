 ...
 */
function crawl(parent, key, url, pathFromRoot, inventory, schema, options) {
  var obj = key === null ? parent : parent[key];

  if (obj && typeof obj === 'object') {
    if ($Ref.is$Ref(obj)) {
      inventory$Ref(parent, key, url, pathFromRoot, inventory, schema, options);
    }
    else {
      ...
      keys.forEach(function(key) {
        ...
        if ($Ref.is$Ref(value)) {
          inventory$Ref(obj, key, url, keyPathFromRoot, inventory, schema, options);
        }
        else {
          crawl(obj, key, keyPath, keyPathFromRoot, inventory, schema, options);
        }
      });
    }
  }
 ...
 * @param {$RefParserOptions} options
 */
function inventory$Ref($refParent, $refKey, url, pathFromRoot, inventory, schema, options) {
  ...
  crawl(pointer.value, null, pointer.url, pathFromRoot, inventory, schema, options);
}
