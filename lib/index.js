...
/**
 * Parses the given JSON schema, expands any `allOf` fields, and builds the composed object.
 * That is, all `allOf` fields are replaced with their composed values.
 *
 * @param {string|object} schema - The file path or URL of the JSON schema. Or a JSON schema object.
 * @param {$RefParserOptions} [options] - Options that determine how the schema is parsed, resolved, and composed
 * @param {function} [callback] - An error-first callback. The second parameter is the composed JSON schema object
 * @returns {Promise} - The returned promise resolves with the composed JSON schema object.
 */
$RefParser.expand = function(schema, options, callback) {
  var Class = this;
  return new Class().expand(schema, options, callback);
};
 ...
 * @returns {Promise} - The returned promise resolves with the composed JSON schema object.
 */
$RefParser.prototype.expand = function(schema, options, callback) {
  var me = this;
  var args = normalizeArgs(arguments);

  return this.resolve(args.schema, args.options)
    .then(function() {
      expand(me, args.options);
      util.doCallback(args.callback, null, me.schema);
      return me.schema;
    })
    .catch(function(err) {
