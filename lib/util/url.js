...
/**
 * Determines whether the given path is a URL (as opposed to a file path).
 *
 * @param   {string} path
 * @returns {boolean}
 */
...
/**
 * Determines whether the given path is an HTTP/HTTPS URL.
 *
 * @param   {string} path
 * @returns {boolean}
 */
exports.isHttpUrl = function isHttpUrl(path) {
  var protocol = protocolPattern.exec(path);
  if (protocol) {
    protocol = protocol[1].toLowerCase();
  ...
  // There's no protocol, but we're running in a browser, so assume HTTP
