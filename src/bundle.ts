...
import { resolve, stripHash, getHash } from './util/url'
...
function inventory$Ref(
  ...
  var $refPath = resolve(path, $ref.$ref)
  var pointer = $refs._resolve($refPath, options)
  var depth = Pointer.parse(pathFromRoot).length
  var file = stripHash(pointer.path)
  var hash = getHash(pointer.path)
...
function remap(inventory) {
  ...
  inventory.forEach(function(entry) {
    ...
    debug(
      '    new value: %s',
      entry.$ref && entry.$ref.$ref ? entry.$ref.$ref : '[object Object]'
    )
  })
}
