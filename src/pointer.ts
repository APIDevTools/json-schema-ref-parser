import $Ref from './ref'
import { getHash, resolve } from './util/url'
...
export default class Pointer {
   ...
   */
  set(obj, value, options?: $RefParserOptions) {
    var tokens = Pointer.parse(this.path)
    var token
  ...
  static parse(path) {
    ...
    var pointer = getHash(path).substr(1)
    ...
    if (pointer[0] !== '') {
      throw ono.syntax(
        'Invalid $ref pointer "%s". Pointers must begin with "#/"',
        pointer
      )
    }
...
function resolveIf$Ref(pointer, options) {
  ...
  if ($Ref.isAllowed$Ref(pointer.value, options)) {
    var $refPath = resolve(pointer.path, pointer.value.$ref)
