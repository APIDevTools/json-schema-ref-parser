 ...
 * @license MIT
 */
"use strict";function bundle(e,r){util.debug("Bundling $ref pointers in %s",e.$refs._basePath);var t=[];crawl(e.schema,e.$refs._basePath+"#","#",t,e.$refs,r),remap(t)}function crawl(e,r,t,i,a,n){if(e&&"object"==typeof e){var f=Object.keys(e),o=f.indexOf("definitions");o>0&&f.splice(0,0,f.splice(o,1)[0]),f.forEach(function(f){var o=Pointer.join(r,f),l=Pointer.join(t,f),h=e[f];$Ref.is$Ref(h)?i.some(function(r){return r.parent===e&&r.key===f})||inventory$Ref(e,f,r,l,i,a,n):crawl(h,o,l,i,a,n)})}}function inventory$Ref(e,r,t,i,a,n,f){var o=e[r],l=url.resolve(t,o.$ref),h=n._resolve(l),s=Pointer.parse(i).length,u=util.path.stripHash(h.path),c=util.path.getHash(h.path),p=u!==n._basePath,d=$Ref.isExtended$Ref(o);a.push({$ref:o,parent:e,key:r,pathFromRoot:i,depth:s,file:u,hash:c,value:h.value,circular:h.circular,extended:d,external:p}),crawl(h.value,h.path,i,a,n,f)}function remap(e){e.sort(function(e,r){return e.file!==r.file?e.file<r.file?-1:1:e.hash!==r.hash?e.hash<r.hash?-1:1:e.circular!==r.circular?e.circular?-1:1:e.extended!==r.extended?e.extended?1:-1:e.depth!==r.depth?e.depth-r.depth:r.pathFromRoot.lastIndexOf("/definitions")-e.pathFromRoot.lastIndexOf("/definitions")});var r,t,i;e.forEach(function(e){util.debug('Re-mapping $ref pointer "%s" at %s',e.$ref.$ref,e.pathFromRoot),e.external?e.file!==r||0!==e.hash.indexOf(t)?(r=e.file,t=e.hash,i=e.pathFromRoot,e.$ref=e.parent[e.key]=$Ref.dereference(e.$ref,e.value),e.circular&&(e.$ref.$ref=e.pathFromRoot)):e.$ref.$ref=Pointer.join(i,Pointer.parse(e.hash)):e.$ref.$ref=e.hash,util.debug("    new value: %s",e.$ref&&e.$ref.$ref?e.$ref.$ref:"[object Object]")})}var $Ref=require("./ref"),Pointer=require("./pointer"),util=require("./util"),url=require("url");module.exports=bundle;

},{"./pointer":10,"./ref":14,"./util":17,"url":95}],2:[function(require,module,exports){
"use strict";function dereference(e,r){util.debug("Dereferencing $ref pointers in %s",e.$refs._basePath),e.$refs.circular=!1,crawl(e.schema,e.$refs._basePath,"#",[],e.$refs,r)}function crawl(e,r,n,c,f,i){var u=!1;return e&&"object"==typeof e&&(c.push(e),Object.keys(e).forEach(function(a){var o=Pointer.join(r,a),t=Pointer.join(n,a),l=e[a],s=!1;if($Ref.is$Ref(l,i)){var d=dereference$Ref(l,o,t,c,f,i);s=d.circular,e[a]=d.value}else s=-1===c.indexOf(l)?crawl(l,o,t,c,f,i):foundCircularReference(o,f,i);u=u||s}),c.pop()),u}function dereference$Ref(e,r,n,c,f,i){util.debug('Dereferencing $ref pointer "%s" at %s',e.$ref,r);var u=url.resolve(r,e.$ref),a=f._resolve(u),o=a.circular,t=o||-1!==c.indexOf(a.value);t&&foundCircularReference(r,f,i);var l=$Ref.dereference(e,a.value);return t||(t=crawl(l,a.path,n,c,f,i)),t&&!o&&"ignore"===i.dereference.circular&&(l=e),o&&(l.$ref=n),{circular:t,value:l}}function foundCircularReference(e,r,n){if(r.circular=!0,!n.dereference.circular)throw ono.reference("Circular $ref pointer found at %s",e);return!0}var $Ref=require("./ref"),Pointer=require("./pointer"),util=require("./util"),ono=require("ono"),url=require("url");module.exports=dereference;

},{"./pointer":10,"./ref":14,"./util":17,"ono":72,"url":95}],3:[function(require,module,exports){
(function (Buffer){
"use strict";function $RefParser(){this.schema=null,this.$refs=new $Refs}function normalizeArgs(e){var r=e[1],t=e[2];return"function"==typeof r&&(t=r,r=void 0),r instanceof Options||(r=new Options(r)),{schema:e[0],options:r,callback:t}}var Promise=require("./util/promise"),Options=require("./options"),$Refs=require("./refs"),$Ref=require("./ref"),read=require("./read"),resolve=require("./resolve"),bundle=require("./bundle"),dereference=require("./dereference"),util=require("./util"),url=require("url"),maybe=require("call-me-maybe"),ono=require("ono");module.exports=$RefParser,module.exports.YAML=require("./util/yaml"),$RefParser.parse=function(e,r,t){var s=this;return(new s).parse(e,r,t)},$RefParser.prototype.parse=function(e,r,t){var s,a=normalizeArgs(arguments);if(a.schema&&"object"==typeof a.schema){this.$refs._basePath="";var n=new $Ref(this.$refs,this.$refs._basePath);n.value=a.schema,s=Promise.resolve({$ref:n})}else{if(!a.schema||"string"!=typeof a.schema){var o=ono("Expected a file path, URL, or object. Got %s",a.schema);return maybe(a.callback,Promise.reject(o))}a.schema=util.path.localPathToUrl(a.schema),a.schema=url.resolve(util.path.cwd(),a.schema),this.$refs._basePath=util.path.stripHash(a.schema),s=read(a.schema,this.$refs,a.options)}var i=this;return s.then(function(e){var r=e.$ref.value;if(!r||"object"!=typeof r||Buffer.isBuffer(r))throw ono.syntax('"%s" is not a valid JSON Schema',i.$refs._basePath||r);return i.schema=r,maybe(a.callback,Promise.resolve(i.schema))})["catch"](function(e){return maybe(a.callback,Promise.reject(e))})},$RefParser.resolve=function(e,r,t){var s=this;return(new s).resolve(e,r,t)},$RefParser.prototype.resolve=function(e,r,t){var s=this,a=normalizeArgs(arguments);return this.parse(a.schema,a.options).then(function(){return resolve(s,a.options)}).then(function(){return maybe(a.callback,Promise.resolve(s.$refs))})["catch"](function(e){return maybe(a.callback,Promise.reject(e))})},$RefParser.bundle=function(e,r,t){var s=this;return(new s).bundle(e,r,t)},$RefParser.prototype.bundle=function(e,r,t){var s=this,a=normalizeArgs(arguments);return this.resolve(a.schema,a.options).then(function(){return bundle(s,a.options),maybe(a.callback,Promise.resolve(s.schema))})["catch"](function(e){return maybe(a.callback,Promise.reject(e))})},$RefParser.dereference=function(e,r,t){var s=this;return(new s).dereference(e,r,t)},$RefParser.prototype.dereference=function(e,r,t){var s=this,a=normalizeArgs(arguments);return this.resolve(a.schema,a.options).then(function(){return dereference(s,a.options),maybe(a.callback,Promise.resolve(s.schema))})["catch"](function(e){return maybe(a.callback,Promise.reject(e))})};
...
"use strict";function $RefParserOptions(e){this.parse={json:parseJSON,yaml:parseYAML,text:parseText,binary:parseBinary},this.resolve={file:readFile,http:readHttp},this.dereference={circular:!0},merge(e,this)}function merge(e,r){if(e)for(var t=Object.keys(e),a=0;a<t.length;a++){var i=t[a],s=e[i],p=r[i];s&&"object"==typeof s&&!Array.isArray(s)?("function"==typeof p&&(p=merge(p,util.bind(p))),r[i]=merge(s,p||{})):r[i]=s}return r}var parseJSON=require("./parse/json"),parseYAML=require("./parse/yaml"),parseText=require("./parse/text"),parseBinary=require("./parse/binary"),readFile=require("./read/file"),readHttp=require("./read/http"),util=require("./util");module.exports=$RefParserOptions;
...
"use strict";function parseBinary(e,r,o){return e}module.exports=parseBinary,module.exports.order=4,module.exports.empty=!0,module.exports.ext=[".jpeg",".jpg",".gif",".png",".bmp",".ico"];

},{}],6:[function(require,module,exports){
(function (Buffer){
"use strict";function parse(e,r,t){return new Promise(function(n,s){function o(n){if(i=f[n],!i)return s(a?ono.syntax(a,'Error parsing "%s"',r):ono.syntax('Unable to parse "%s"',r));try{util.debug('Parsing "%s" as %s',r,i.name),u=i.fn(e,r,t)}catch(c){util.debug("    ",c.message),a=c,o(n+1)}}var i,u,a,f=getSortedParsers(r,t);util.debug('These parsers match "%s":\n',r,f),o(0),!i.fn.empty&&isEmpty(u)?s(ono.syntax('Error parsing "%s" as %s. \nParsed value is empty',r,i.name)):n(u)})}function getSortedParsers(e,r){var t=util.path.extname(e),n=3;return util.orderedFunctions(r.parse).map(function(r){var s={score:score(e,t,r.fn.ext),order:r.order,name:r.name,fn:r.fn};return n=Math.min(s.score,n),s}).filter(function(e){return 3===n||e.score<3}).sort(function(e,r){return e.score-r.score})}function score(e,r,t){for(var n=3,s=0;s<t.length;s++){var o=t[s];if(o===r)return 1;o instanceof RegExp&&o.test(e)&&(n=Math["in"](n,2))}return n}function isEmpty(e){return!e||"object"==typeof e&&0===Object.keys(e).length||"string"==typeof e&&0===e.trim().length||Buffer.isBuffer(e)&&0===e.length}var util=require("../util"),Promise=require("../util/promise"),ono=require("ono");module.exports=parse;

}).call(this,{"isBuffer":require("../../node_modules/is-buffer/index.js")})

},{"../../node_modules/is-buffer/index.js":39,"../util":17,"../util/promise":19,"ono":72}],7:[function(require,module,exports){
"use strict";function parseJSON(e,r,o){return JSON.parse(e.toString())}module.exports=parseJSON,module.exports.order=1,module.exports.empty=!0,module.exports.ext=[".json"];

},{}],8:[function(require,module,exports){
"use strict";function parseText(e,t,s){return e.toString(s.parse.text.encoding)}module.exports=parseText,module.exports.order=3,module.exports.empty=!0,module.exports.encoding="utf8",module.exports.ext=[".txt",".htm",".html",".md",".xml",".js",".min",".map",".css",".scss",".less",".svg"];

},{}],9:[function(require,module,exports){
"use strict";function parseYAML(e,r,t){return YAML.parse(e.toString())}var YAML=require("../util/yaml");module.exports=parseYAML,module.exports.order=2,module.exports.empty=!0,module.exports.ext=[".yaml",".yml",".json"];

},{"../util/yaml":20}],10:[function(require,module,exports){
"use strict";function Pointer(e,r){this.$ref=e,this.path=r,this.value=void 0,this.circular=!1}function resolveIf$Ref(e){if($Ref.is$Ref(e.value)){var r=url.resolve(e.path,e.value.$ref);if(r!==e.path){var t=e.$ref.$refs._resolve(r);return $Ref.isExtended$Ref(e.value)?e.value=$Ref.dereference(e.value,t.value):(e.$ref=t.$ref,e.path=t.path,e.value=t.value),!0}e.circular=!0}}function setValue(e,r,t){if(!e.value||"object"!=typeof e.value)throw ono.syntax('Error assigning $ref pointer "%s". \nCannot set "%s" of a non-object.',e.path,r);return"-"===r&&Array.isArray(e.value)?e.value.push(t):e.value[r]=t,t}module.exports=Pointer;var $Ref=require("./ref"),util=require("./util"),url=require("url"),ono=require("ono"),slashes=/\//g,tildes=/~/g,escapedSlash=/~1/g,escapedTilde=/~0/g;Pointer.prototype.resolve=function(e){var r=Pointer.parse(this.path);this.value=e;for(var t=0;t<r.length;t++){resolveIf$Ref(this)&&(this.path=Pointer.join(this.path,r.slice(t)));var i=r[t];if(void 0===this.value[i])throw ono.syntax('Error resolving $ref pointer "%s". \nToken "%s" does not exist.',this.path,i);this.value=this.value[i]}return resolveIf$Ref(this),this},Pointer.prototype.set=function(e,r){var t,i=Pointer.parse(this.path);if(0===i.length)return this.value=r,r;this.value=e;for(var s=0;s<i.length-1;s++)resolveIf$Ref(this),t=i[s],this.value&&void 0!==this.value[t]?this.value=this.value[t]:this.value=setValue(this,t,{});return resolveIf$Ref(this),t=i[i.length-1],setValue(this,t,r),e},Pointer.parse=function(e){var r=util.path.getHash(e).substr(1);if(!r)return[];r=r.split("/");for(var t=0;t<r.length;t++)r[t]=decodeURI(r[t].replace(escapedSlash,"/").replace(escapedTilde,"~"));if(""!==r[0])throw ono.syntax('Invalid $ref pointer "%s". Pointers must begin with "#/"',r);return r.slice(1)},Pointer.join=function(e,r){-1===e.indexOf("#")&&(e+="#"),r=Array.isArray(r)?r:[r];for(var t=0;t<r.length;t++){var i=r[t];e+="/"+encodeURI(i.replace(tildes,"~0").replace(slashes,"~1"))}return e};

},{"./ref":14,"./util":17,"ono":72,"url":95}],11:[function(require,module,exports){
(function (process){
"use strict";function readFile(e,r){return process.browser||util.path.isUrl(e)?void 0:new Promise(function(r,o){var i;try{i=util.path.urlToLocalPath(e)}catch(t){o(ono.uri(t,"Malformed URI: %s",e))}util.debug("Opening file: %s",i);try{fs.readFile(i,function(e,t){e?o(ono(e,'Error opening file "%s"',i)):r(t)})}catch(t){o(ono(t,'Error opening file "%s"',i))}})}var fs=require("fs"),ono=require("ono"),Promise=require("../util/promise"),util=require("../util");module.exports=readFile,module.exports.order=1,module.exports.cache=6e4;

}).call(this,require('_process'))

},{"../util":17,"../util/promise":19,"_process":74,"fs":23,"ono":72}],12:[function(require,module,exports){
(function (process,Buffer){
"use strict";function readHttp(e,t){var o=url.parse(e);return process.browser&&!o.protocol&&(o.protocol=url.parse(location.href).protocol),"http:"===o.protocol||"https:"===o.protocol?download(o,t):void 0}function download(e,t,o){return new Promise(function(r,s){e=url.parse(e),o=o||[],o.push(e.href),get(e,t).then(function(n){if(n.statusCode>=400)throw ono({status:n.statusCode},"HTTP ERROR %d",n.statusCode);if(n.statusCode>=300)if(o.length>t.resolve.http.redirects)s(ono({status:n.statusCode},"Error downloading %s. \nToo many redirects: \n  %s",o[0],o.join(" \n  ")));else{if(!n.headers.location)throw ono({status:n.statusCode},"HTTP %d redirect with no location header",n.statusCode);util.debug("HTTP %d redirect %s -> %s",n.statusCode,e.href,n.headers.location);var u=url.resolve(e,n.headers.location);download(u,t,o).then(r,s)}else r(n.body||new Buffer(0))})["catch"](function(t){s(ono(t,"Error downloading",e.href))})})}function get(e,t){return new Promise(function(o,r){util.debug("GET",e.href);var s="https:"===e.protocol?https:http,n=s.get({hostname:e.hostname,port:e.port,path:e.path,auth:e.auth,headers:t.resolve.http.headers||{},withCredentials:t.resolve.http.withCredentials});"function"==typeof n.setTimeout&&n.setTimeout(t.resolve.http.timeout),n.on("timeout",function(){n.abort()}),n.on("error",r),n.once("response",function(e){e.body=new Buffer(0),e.on("data",function(t){e.body=Buffer.concat([e.body,new Buffer(t)])}),e.on("error",r),e.on("end",function(){o(e)})})})}var http=require("http"),https=require("https"),url=require("url"),util=require("../util"),Promise=require("../util/promise"),ono=require("ono");module.exports=readHttp,module.exports.order=2,module.exports.cache=3e5,module.exports.headers=null,module.exports.timeout=5e3,module.exports.redirects=5,module.exports.withCredentials=!1;

}).call(this,require('_process'),require("buffer").Buffer)

},{"../util":17,"../util/promise":19,"_process":74,"buffer":24,"http":90,"https":34,"ono":72,"url":95}],13:[function(require,module,exports){
"use strict";function read(e,r,t){try{e=util.path.stripHash(e),util.debug("Reading %s",e);var n=r._get$Ref(e);return n&&!n.isExpired()?(util.debug("    cached from %s",n.pathType),Promise.resolve({$ref:n,cached:!0})):(n=new $Ref(r,e),readers(e,t).then(function(r){return n.pathType=r.pathType,parse(r.data,e,t)}).then(function(e){return n.value=e,{$ref:n,cached:!1}}))}catch(i){return Promise.reject(i)}}function readers(e,r){return new Promise(function(t,n){var i,u;util.orderedFunctions(r.resolve).some(function(t){return i=t.fn(e,r),i?(u=t.name,!0):void 0}),i?i.then(function(e){t({data:e,pathType:u})},n):n(ono.syntax('Unable to resolve $ref pointer "%s"',e))})}var parse=require("../parse"),$Ref=require("../ref"),util=require("../util"),Promise=require("../util/promise"),ono=require("ono");module.exports=read;

},{"../parse":6,"../ref":14,"../util":17,"../util/promise":19,"ono":72}],14:[function(require,module,exports){
"use strict";function $Ref(e,t){t=util.path.stripHash(t),e._$refs[t]=this,this.$refs=e,this.path=t,this.pathType=void 0,this.value=void 0,this.expires=void 0}module.exports=$Ref;var Pointer=require("./pointer"),util=require("./util");$Ref.prototype.isExpired=function(){return!!(this.expires&&this.expires<=Date.now())},$Ref.prototype.expire=function(){this.expires=new Date},$Ref.prototype.setExpiration=function(e){var t=e.resolve[this.pathType];if(t){var i=t.cache;if(i>0){var r=Date.now()+i;this.expires=new Date(r)}else this.expires=new Date}else this.expires=void 0},$Ref.prototype.exists=function(e){try{return this.resolve(e),!0}catch(t){return!1}},$Ref.prototype.get=function(e){return this.resolve(e).value},$Ref.prototype.resolve=function(e){var t=new Pointer(this,e);return t.resolve(this.value)},$Ref.prototype.set=function(e,t){var i=new Pointer(this,e);this.value=i.set(this.value,t)},$Ref.is$Ref=function(e){return e&&"object"==typeof e&&"string"==typeof e.$ref&&e.$ref.length>0},$Ref.isExternal$Ref=function(e){return $Ref.is$Ref(e)&&"#"!==e.$ref[0]},$Ref.isExtended$Ref=function(e){return $Ref.is$Ref(e)&&Object.keys(e).length>1},$Ref.dereference=function(e,t){if(t&&"object"==typeof t&&$Ref.isExtended$Ref(e)){var i={};return Object.keys(e).forEach(function(t){"$ref"!==t&&(i[t]=e[t])}),Object.keys(t).forEach(function(e){e in i||(i[e]=t[e])}),i}return t};

},{"./pointer":10,"./util":17}],15:[function(require,module,exports){
"use strict";function $Refs(){this.circular=!1,this._basePath="",this._$refs={}}function getPaths(e,t){var r=Object.keys(e);return t=Array.isArray(t[0])?t[0]:Array.prototype.slice.call(t),t.length>0&&t[0]&&(r=r.filter(function(r){return-1!==t.indexOf(e[r].pathType)})),r.map(function(t){return{encoded:t,decoded:"file"===e[t].pathType?util.path.urlToLocalPath(t,!0):t}})}var util=require("./util"),url=require("url"),ono=require("ono");module.exports=$Refs,$Refs.prototype.paths=function(e){var t=getPaths(this._$refs,arguments);return t.map(function(e){return e.decoded})},$Refs.prototype.values=function(e){var t=this._$refs,r=getPaths(t,arguments);return r.reduce(function(e,r){return e[r.decoded]=t[r.encoded].value,e},{})},$Refs.prototype.toJSON=$Refs.prototype.values,$Refs.prototype.isExpired=function(e){var t=this._get$Ref(e);return void 0===t||t.isExpired()},$Refs.prototype.expire=function(e){var t=this._get$Ref(e);t&&t.expire()},$Refs.prototype.exists=function(e){try{return this._resolve(e),!0}catch(t){return!1}},$Refs.prototype.get=function(e){return this._resolve(e).value},$Refs.prototype.set=function(e,t){e=url.resolve(this._basePath,e);var r=util.path.stripHash(e),s=this._$refs[r];if(!s)throw ono('Error resolving $ref pointer "%s". \n"%s" not found.',e,r);s.set(e,t)},$Refs.prototype._resolve=function(e){e=url.resolve(this._basePath,e);var t=util.path.stripHash(e),r=this._$refs[t];if(!r)throw ono('Error resolving $ref pointer "%s". \n"%s" not found.',e,t);return r.resolve(e)},$Refs.prototype._get$Ref=function(e){e=url.resolve(this._basePath,e);var t=util.path.stripHash(e);return this._$refs[t]},$Refs.prototype._setExpirations=function(e){var t=this;Object.keys(this._$refs).forEach(function(r){var s=t._$refs[r];s.setExpiration(e)})};

},{"./util":17,"ono":72,"url":95}],16:[function(require,module,exports){
"use strict";function resolve(e,r){try{util.debug("Resolving $ref pointers in %s",e.$refs._basePath);var t=crawl(e.schema,e.$refs._basePath+"#",e.$refs,r);return Promise.all(t).then(function(){e.$refs._setExpirations(r)})["catch"](function(t){throw e.$refs._setExpirations(r),t})}catch(i){return e.$refs._setExpirations(r),Promise.reject(i)}}function crawl(e,r,t,i){var s=[];return e&&"object"==typeof e&&Object.keys(e).forEach(function(n){var o=Pointer.join(r,n),a=e[n];if($Ref.isExternal$Ref(a)){var u=resolve$Ref(a,o,t,i);s.push(u)}else s=s.concat(crawl(a,o,t,i))}),s}function resolve$Ref(e,r,t,i){util.debug('Resolving $ref pointer "%s" at %s',e.$ref,r);var s=url.resolve(r,e.$ref);return read(s,t,i).then(function(e){if(!e.cached){util.debug("Resolving $ref pointers in %s",e.$ref.path);var r=crawl(e.$ref.value,e.$ref.path+"#",t,i);return Promise.all(r)}})}var Promise=require("./util/promise"),$Ref=require("./ref"),Pointer=require("./pointer"),read=require("./read"),util=require("./util"),url=require("url");module.exports=resolve;

},{"./pointer":10,"./read":13,"./ref":14,"./util":17,"./util/promise":19,"url":95}],17:[function(require,module,exports){
"use strict";var debug=require("debug"),path=require("./path");exports.debug=debug("json-schema-ref-parser"),exports.path=path,exports.bind=function(r,e){return function(){return r.apply(e,arguments)}},exports.orderedFunctions=function(r){return Object.keys(r).map(function(e){return{order:r[e].order,name:e,fn:r[e]}}).filter(function(r){return"function"==typeof r.fn}).sort(function(r,e){return r.order-e.order})};
...
},{"./_stream_duplex":80,"_process":74,"buffer":24,"core-util-is":28,"events":33,"inherits":38,"isarray":40,"process-nextick-args":73,"stream":89,"string_decoder/":94,"util":22}],83:[function(require,module,exports){
...
},{"./_stream_duplex":80,"buffer":24,"core-util-is":28,"events":33,"inherits":38,"process-nextick-args":73,"stream":89,"util-deprecate":97}],85:[function(require,module,exports){
...
},{"./lib/_stream_duplex.js":80,"./lib/_stream_passthrough.js":81,"./lib/_stream_readable.js":82,"./lib/_stream_transform.js":83,"./lib/_stream_writable.js":84,"stream":89}],87:[function(require,module,exports){
