...
"use strict";function dereference(e,r){debug("Dereferencing $ref pointers in %s",e.rootUrl);var c=crawl(e.root,e.rootUrl,"#",[],e,r);e.circular=c.circular,e.files[0].data=c.value;for(var u=0;u<e.files.length;u++)e.files[u].dereferenced=!0;return e}function crawl(e,r,c,u,i,n){var l,f={value:e,circular:!1};return e&&"object"==typeof e&&(u.push(e),$Ref.isAllowed(e,n)?(l=dereference$Ref(e,r,c,u,i,n),f.circular=l.circular,f.value=l.value):Object.keys(e).forEach(function(a){var o=Pointer.join(r,a),t=Pointer.join(c,a),d=e[a],v=!1;$Ref.isAllowed(d,n)?(l=dereference$Ref(d,o,t,u,i,n),v=l.circular,e[a]=l.value):-1===u.indexOf(d)?(l=crawl(d,o,t,u,i,n),v=l.circular,e[a]=l.value):v=foundCircularReference(o,i,n),f.circular=f.circular||v}),u.pop()),f}function dereference$Ref(e,r,c,u,i,n){debug('Dereferencing $ref pointer "%s" at %s',e.$ref,r);var l=URL.resolve(r,e.$ref),f=Pointer.resolve(l,i,n),a=f.circular,o=a||-1!==u.indexOf(f.value);o&&foundCircularReference(r,i,n);var t=$Ref.dereference(e,f.value);if(!o){var d=crawl(t,f.url,c,u,i,n);o=d.circular,t=d.value}return o&&!a&&"ignore"===n.dereference.circular&&(t=e),a&&(t.$ref=c),{circular:o,value:t}}function foundCircularReference(e,r,c){if(r.circular=!0,!c.dereference.circular)throw ono.reference("Circular $ref pointer found at %s",e);return!0}var $Ref=require("./ref"),Pointer=require("./pointer"),ono=require("ono"),debug=require("./util/debug"),URL=require("./util/url");module.exports=dereference;

},{"./pointer":13,"./ref":17,"./util/debug":20,"./util/url":23,"ono":71}],3:[function(require,module,exports){
"use strict";function FileArray(){var r=[];return r.exists=fileExists,r.get=getFile,r}function fileExists(r){if(0===this.length)return!1;var e=getFileUrl(this,r);return!!findFile(this,e)}function getFile(r){if(0===this.length)throw ono('Error resolving "%s". \nThe schema is empty.',r);var e=getFileUrl(this,r),i=findFile(this,e);if(!i)throw ono('Error resolving "%s". \nThe file (%s) was not found in the schema.',r,e);return i}function getFileUrl(r,e){var i=URL.stripHash(e);return URL.resolve(r[0].url,i)}function findFile(r,e){for(var i=0;i<r.length;i++)if(r[i].url===e)return r[i]}var ono=require("ono"),URL=require("./util/url");module.exports=FileArray;

},{"./util/url":23,"ono":71}],4:[function(require,module,exports){
"use strict";function File(e){e=URL.stripHash(e),this.url=e,this.urlType=void 0,this.path=decodeURI(e),this.extension=URL.getExtension(e),this.data=void 0,this.dataType=void 0,this.parsed=!1,this.dereferenced=!1}var URL=require("./util/url");module.exports=File;

},{"./util/url":23}],5:[function(require,module,exports){
"use strict";function normalizeArgs(e){var r,n,t,o;if(e=Array.prototype.slice.call(e),"function"==typeof e[e.length-1]&&(o=e.pop()),"string"==typeof e[0]&&"{"!==e[0].trim()[0]&&-1===e[0].indexOf("\n")?(r=e[0],"object"==typeof e[2]?(n=e[1],t=e[2]):(n=void 0,t=e[1])):(r="",n=e[0],t=e[1]),!r&&!n){var a=ono("Invalid arguments. Expected a URL, file path, JSON/YAML string, or object.");return maybe(e.callback,Promise.reject(a))}return r&&(URL.isFileSystemPath(r)&&(r=URL.fromFileSystemPath(r)),r=URL.resolve(URL.cwd(),r)),t instanceof Options||(t=new Options(t)),{url:r,data:n,options:t,callback:o}}var maybe=require("call-me-maybe"),ono=require("ono"),Options=require("./options"),resolve=require("./resolve"),parse=require("./parse"),bundle=require("./bundle"),dereference=require("./dereference"),URL=require("./util/url");exports.parse=function(e,r,n,t){var o=normalizeArgs(arguments);return maybe(o.callback,parse(o.url,o.data,o.options))},exports.resolve=function(e,r,n,t){var o=normalizeArgs(arguments);return maybe(o.callback,parse(o.url,o.data,o.options).then(function(e){return resolve(e,o.options)}))},exports.bundle=function(e,r,n,t){var o=normalizeArgs(arguments);return maybe(o.callback,parse(o.url,o.data,o.options).then(function(e){return resolve(e,o.options)}).then(function(e){return bundle(e,o.options)}))},exports.dereference=function(e,r,n,t){var o=normalizeArgs(arguments);return maybe(o.callback,parse(o.url,o.data,o.options).then(function(e){return resolve(e,o.options)}).then(function(e){return dereference(e,o.options)}))};
...
"use strict";function $RefParserOptions(e){merge(this,$RefParserOptions.defaults),merge(this,e)}function merge(e,r){if(isMergeable(r))for(var a=Object.keys(r),s=0;s<a.length;s++){var t=a[s],i=r[t],n=e[t];isMergeable(i)?e[t]=merge(n||{},i):void 0!==i&&(e[t]=i)}return e}function isMergeable(e){return e&&"object"==typeof e&&!Array.isArray(e)&&!(e instanceof RegExp)&&!(e instanceof Date)}var jsonParser=require("./parsers/json"),yamlParser=require("./parsers/yaml"),textParser=require("./parsers/text"),binaryParser=require("./parsers/binary"),fileReader=require("./readers/file"),httpReader=require("./readers/http");module.exports=$RefParserOptions,$RefParserOptions.defaults={read:{file:fileReader,http:httpReader},parse:{json:jsonParser,yaml:yamlParser,text:textParser,binary:binaryParser},resolve:{external:!0},dereference:{circular:!0}};
...
(function (Buffer){
"use strict";function parseFile(r,e){return new Promise(function(n,s){function u(e){debug("%s was parsed as %s",r.url,e.plugin.name),!e.plugin.allowEmpty&&isEmpty(e.result)&&s(ono.syntax('Error parsing "%s" as %s. \nParsed value is empty',r.url,e.plugin.name)),r.data=e.result,r.dataType=e.plugin.name,r.parsed=!0,n(r)}function i(e){e?(e=e instanceof Error?e:new Error(e),s(ono.syntax(e,"Error parsing %s",r.url))):s(ono.syntax("Unable to parse %s",r.url))}debug("Parsing %s",r.url);var t=plugins.all(e.parse),l=plugins.filter(t,"canParse",r),a=l.length>0?l:t;plugins.sort(a),plugins.run(a,"parse",r).then(u,i)})}function isEmpty(r){return void 0===r||"object"==typeof r&&0===Object.keys(r).length||"string"==typeof r&&0===r.trim().length||Buffer.isBuffer(r)&&0===r.length}var ono=require("ono"),debug=require("./util/debug"),plugins=require("./util/plugins"),Promise=require("./util/promise");module.exports=parseFile;
...
"use strict";function parse(e,r,a){return new Promise(function(i,t){function n(){!o.data||"object"!=typeof o.data||Array.isArray(o.data)?t(ono.syntax('"%s" is not a valid JSON Schema',e||o.data)):i(s)}var s=new Schema,o=new File(e);s.files.push(o),"string"==typeof r?(o.data=r,parseFile(o,a).then(n)):r?(o.data=r,o.parsed=!0,n()):readFile(o,a).then(function(){return parseFile(o,a)}).then(n)})}var Schema=require("./schema"),File=require("./file"),Promise=require("./util/promise"),readFile=require("./read-file"),parseFile=require("./parse-file"),ono=require("ono");module.exports=parse;
...
(function (Buffer){
"use strict";var Promise=require("../util/promise"),yaml=require("js-yaml"),ono=require("ono");module.exports={order:200,allowEmpty:!0,canParse:[".yaml",".yml",".json"],parse:function(r){var e;return new Promise(function(o,a){var s=r.data;if(Buffer.isBuffer(s)&&(s=s.toString()),"string"==typeof s){try{e=yaml.safeLoad(s)}catch(t){throw t instanceof Error?t:ono(t,t.message)}o(e)}else o(s)})}};

}).call(this,{"isBuffer":require("../../node_modules/is-buffer/index.js")})

},{"../../node_modules/is-buffer/index.js":38,"../util/promise":22,"js-yaml":40,"ono":71}],13:[function(require,module,exports){
"use strict";function Pointer(){this.file=null,this.path="",this.value=void 0,this.circular=!1}function resolveIf$Ref(e,r,t){if($Ref.isAllowed(e.value,t)){var i=URL.resolve(e.file.url,e.value.$ref);if(i===e.url)return void(e.circular=!0);var l=Pointer.resolve(i,r,t);if(e.value=$Ref.dereference(e.value,l.value),e.value===l.value)return e.file=l.file,e.path=l.path,!0}}function setValue(e,r,t){if(!e.value||"object"!=typeof e.value)throw ono.syntax('Error assigning $ref pointer "%s". \nCannot set "%s" of a non-object.',e.url,r);return"-"===r&&Array.isArray(e.value)?e.value.push(t):e.value[r]=t,t}var $Ref=require("./ref"),URL=require("./util/url"),ono=require("ono"),slashes=/\//g,tildes=/~/g,escapedSlash=/~1/g,escapedTilde=/~0/g;module.exports=Pointer,Object.defineProperties(Pointer.prototype,{url:{configurable:!0,enumerable:!0,get:function(){return this.file.url+this.path}}}),Pointer.resolve=function(e,r,t){var i=new Pointer;i.file=r.files.get(e),i.path=URL.getHash(e);for(var l=(i.file.data,Pointer.parse(i.path)),a=0;a<l.length;a++){resolveIf$Ref(i,r,t)&&(i.path=Pointer.join(i.path,l.slice(a)));var n=l[a];if(void 0===i.value[n])throw ono.syntax('Error resolving $ref pointer "%s". \nToken "%s" does not exist.',i.url,n);i.value=i.value[n]}return resolveIf$Ref(i,r,t),i},Pointer.prototype.set=function(e,r,t){var i,l=Pointer.parse(this.url);if(0===l.length)return this.value=r,r;this.value=e;for(var a=0;a<l.length-1;a++)resolveIf$Ref(this,t),i=l[a],this.value&&void 0!==this.value[i]?this.value=this.value[i]:this.value=setValue(this,i,{});return resolveIf$Ref(this,t),i=l[l.length-1],setValue(this,i,r),e},Pointer.parse=function(e){var r=URL.getHash(e).substr(1);if(!r)return[];r=r.split("/");for(var t=0;t<r.length;t++)r[t]=decodeURI(r[t].replace(escapedSlash,"/").replace(escapedTilde,"~"));if(""!==r[0])throw ono.syntax('Invalid $ref pointer "%s". Pointers must begin with "#/"',r);return r.slice(1)},Pointer.join=function(e,r){-1===e.indexOf("#")&&(e+="#"),r=Array.isArray(r)?r:[r];for(var t=0;t<r.length;t++){var i=r[t];e+="/"+encodeURI(i.replace(tildes,"~0").replace(slashes,"~1"))}return e};

},{"./ref":17,"./util/url":23,"ono":71}],14:[function(require,module,exports){
"use strict";function readFile(e,u){return new Promise(function(r,n){function i(u){debug("%s was read as a %s path",e.url,u.plugin.name),e.urlType=u.plugin.name,e.data=u.result,r(e)}function l(u){n(ono.syntax(u,'Unable to read "%s"',e.url))}debug("Resolving %s",e.url);var s=plugins.all(u.read);s=plugins.filter(s,"canRead",e),plugins.sort(s),plugins.run(s,"read",e).then(i,l)})}var ono=require("ono"),debug=require("./util/debug"),plugins=require("./util/plugins"),Promise=require("./util/promise");module.exports=readFile;

},{"./util/debug":20,"./util/plugins":21,"./util/promise":22,"ono":71}],15:[function(require,module,exports){
"use strict";var fs=require("fs"),ono=require("ono"),Promise=require("../util/promise"),URL=require("../util/url"),debug=require("../util/debug");module.exports={order:100,canRead:function(e){return URL.isFileSystemPath(e.url)},read:function(e){return new Promise(function(r,i){try{e.path=URL.toFileSystemPath(e.url)}catch(o){i(ono.uri(o,"Malformed URI: %s",e.url))}debug("Opening file: %s",e.path);try{fs.readFile(e.path,function(o,t){o?i(ono(o,'Error opening file "%s"',e.path)):r(t)})}catch(o){i(ono(o,'Error opening file "%s"',e.path))}})}};
...
"use strict";var ono=require("ono"),Pointer=require("./pointer"),URL=require("./util/url"),$Ref=exports;$Ref.is$Ref=function(e){return e&&"object"==typeof e&&"string"==typeof e.$ref&&e.$ref.length>0},$Ref.isExternal=function(e){return $Ref.is$Ref(e)&&"#"!==e.$ref[0]},$Ref.isAllowed=function(e,r){return!$Ref.is$Ref(e)||"#"!==e.$ref[0]&&r&&!r.resolve.external?void 0:!0},$Ref.isExtended=function(e){return $Ref.is$Ref(e)&&Object.keys(e).length>1},$Ref.dereference=function(e,r){if(!r||"object"!=typeof r)return r;if(!$Ref.isExtended(e))return r;var t={};return Object.keys(e).forEach(function(r){"$ref"!==r&&(t[r]=e[r])}),Object.keys(r).forEach(function(e){e in t||(t[e]=r[e])}),t};

},{"./pointer":13,"./util/url":23,"ono":71}],18:[function(require,module,exports){
"use strict";function resolve(e,r){if(!r.resolve.external)return Promise.resolve(e);var i;try{debug("Resolving $ref pointers in %s",e.rootUrl),i=crawl(e.root,e.rootUrl+"#",e,r)}catch(t){return Promise.reject(t)}return Promise.all(i).then(function(){return e})}function crawl(e,r,i,t){var o=[];return e&&"object"==typeof e&&($Ref.isExternal(e)?o.push(resolve$Ref(e,r,i,t)):Object.keys(e).forEach(function(n){var l=Pointer.join(r,n),s=e[n];$Ref.isExternal(s)?o.push(resolve$Ref(s,l,i,t)):o=o.concat(crawl(s,l,i,t))})),o}function resolve$Ref(e,r,i,t){debug('Resolving $ref pointer "%s" at %s',e.$ref,r);var o=URL.resolve(r,e.$ref);if(i.files.exists(o))return Promise.resolve();var n=new File(o);return i.files.push(n),readFile(n,t).then(function(){return parseFile(n,t)}).then(function(){debug("Resolving $ref pointers in %s",n.url);var e=crawl(n.data,n.url+"#",i,t);return Promise.all(e)})}var Promise=require("./util/promise"),$Ref=require("./ref"),Pointer=require("./pointer"),readFile=require("./read-file"),parseFile=require("./parse-file"),debug=require("./util/debug"),URL=require("./util/url");module.exports=resolve;

},{"./parse-file":7,"./pointer":13,"./read-file":14,"./ref":17,"./util/debug":20,"./util/promise":22,"./util/url":23}],19:[function(require,module,exports){
"use strict";function Schema(){this.files=new FileArray,this.circular=!1}var FileArray=require("./file-array");module.exports=Schema,Object.defineProperties(Schema.prototype,{root:{configurable:!0,enumerable:!0,get:function(){return 0===this.files.length?null:this.files[0].data}},rootUrl:{configurable:!0,enumerable:!0,get:function(){return 0===this.files.length?null:this.files[0].url}},rootFile:{configurable:!0,enumerable:!0,get:function(){return this.files[0]||null}}}),Schema.prototype.exists=function(e){},Schema.prototype.get=function(e){},Schema.prototype.set=function(e,t){};
...
"use strict";function getResult(e,n,r,t){var u=e[n];if("function"==typeof u)return u.apply(e,[r,t]);if(!t){if(u instanceof RegExp)return u.test(r.url);if("string"==typeof u)return u===r.extension;if(Array.isArray(u))return-1!==u.indexOf(r.extension)}return u}var Promise=require("./promise"),debug=require("./debug");exports.all=function(e){return Object.keys(e).filter(function(n){return"object"==typeof e[n]}).map(function(n){return e[n].name=n,e[n]})},exports.filter=function(e,n,r){return e.filter(function(e){return!!getResult(e,n,r)})},exports.sort=function(e){return e.forEach(function(e){e.order=e.order||Number.MAX_SAFE_INTEGER}),e.sort(function(e,n){return e.order-n.order})},exports.run=function(e,n,r){var t,u,i=0;return new Promise(function(o,s){function f(){if(t=e[i++],!t)return debug("No plug-ins were able to handle",r.url),s(u);try{debug("Running the %s plug-in on %s",t.name,r.url);var o=getResult(t,n,r,l);o&&"function"==typeof o.then?(debug("The %s plug-in returned a promise",t.name),o.then(a,c)):void 0!==o?(debug("The %s plug-in returned synchronously",t.name),a(o)):debug("The %s plug-in did not return a value. Waiting for it to call the callback function",t.name)}catch(f){c(f)}}function l(e,n){debug("The %s plug-in returned asynchronously",t.name),e?c(e):a(n)}function a(e){debug("The %s plug-in was able to handle %s",t.name,r.url),o({plugin:t,result:e})}function c(e){debug("The %s plug-in threw an error for %s\n%s",t.name,r.url,e),u=e,f()}f()})};
...
},{"./_stream_duplex":79,"_process":73,"buffer":27,"core-util-is":30,"events":34,"inherits":37,"isarray":39,"process-nextick-args":72,"stream":88,"string_decoder/":93,"util":25}],82:[function(require,module,exports){
...
},{"./_stream_duplex":79,"_process":73,"buffer":27,"core-util-is":30,"events":34,"inherits":37,"process-nextick-args":72,"stream":88,"util-deprecate":97}],84:[function(require,module,exports){
...
},{"./lib/_stream_duplex.js":79,"./lib/_stream_passthrough.js":80,"./lib/_stream_readable.js":81,"./lib/_stream_transform.js":82,"./lib/_stream_writable.js":83,"stream":88}],86:[function(require,module,exports){
