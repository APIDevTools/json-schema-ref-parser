 ...
 * @license MIT
 */
"use strict";function bundle(e,r){debug("Bundling $ref pointers in %s",e.$refs._basePath);var t=[];crawl(e.schema,e.$refs._basePath+"#","#",t,e.$refs,r),remap(t)}function crawl(e,r,t,a,n,i){if(e&&"object"==typeof e){var f=Object.keys(e),o=f.indexOf("definitions");o>0&&f.splice(0,0,f.splice(o,1)[0]),f.forEach(function(f){var o=Pointer.join(r,f),s=Pointer.join(t,f),u=e[f];$Ref.is$Ref(u)?a.some(function(r){return r.parent===e&&r.key===f})||inventory$Ref(e,f,r,s,a,n,i):crawl(u,o,s,a,n,i)})}}function inventory$Ref(e,r,t,a,n,i,f){var o=e[r],s=url.resolve(t,o.$ref),u=i._resolve(s,f),h=Pointer.parse(a).length,l=url.stripHash(u.path),c=url.getHash(u.path),d=l!==i._basePath,p=$Ref.isExtended$Ref(o);n.push({$ref:o,parent:e,key:r,pathFromRoot:a,depth:h,file:l,hash:c,value:u.value,circular:u.circular,extended:p,external:d}),crawl(u.value,u.path,a,n,i,f)}function remap(e){e.sort(function(e,r){return e.file!==r.file?e.file<r.file?-1:1:e.hash!==r.hash?e.hash<r.hash?-1:1:e.circular!==r.circular?e.circular?-1:1:e.extended!==r.extended?e.extended?1:-1:e.depth!==r.depth?e.depth-r.depth:r.pathFromRoot.lastIndexOf("/definitions")-e.pathFromRoot.lastIndexOf("/definitions")});var r,t,a;e.forEach(function(e){debug('Re-mapping $ref pointer "%s" at %s',e.$ref.$ref,e.pathFromRoot),e.external?e.file!==r||0!==e.hash.indexOf(t)?(r=e.file,t=e.hash,a=e.pathFromRoot,e.$ref=e.parent[e.key]=$Ref.dereference(e.$ref,e.value),e.circular&&(e.$ref.$ref=e.pathFromRoot)):e.$ref.$ref=Pointer.join(a,Pointer.parse(e.hash)):e.$ref.$ref=e.hash,debug("    new value: %s",e.$ref&&e.$ref.$ref?e.$ref.$ref:"[object Object]")})}var $Ref=require("./ref"),Pointer=require("./pointer"),debug=require("./util/debug"),url=require("./util/url");module.exports=bundle;

},{"./pointer":10,"./ref":11,"./util/debug":16,"./util/url":19}],2:[function(require,module,exports){
"use strict";function dereference(e,r){debug("Dereferencing $ref pointers in %s",e.$refs._basePath),e.$refs.circular=!1,crawl(e.schema,e.$refs._basePath,"#",[],e.$refs,r)}function crawl(e,r,n,c,f,u){var i=!1;return e&&"object"==typeof e&&(c.push(e),Object.keys(e).forEach(function(o){var a=Pointer.join(r,o),l=Pointer.join(n,o),t=e[o],s=!1;if($Ref.isAllowed$Ref(t,u)){var d=dereference$Ref(t,a,l,c,f,u);s=d.circular,e[o]=d.value}else s=-1===c.indexOf(t)?crawl(t,a,l,c,f,u):foundCircularReference(a,f,u);i=i||s}),c.pop()),i}function dereference$Ref(e,r,n,c,f,u){debug('Dereferencing $ref pointer "%s" at %s',e.$ref,r);var i=url.resolve(r,e.$ref),o=f._resolve(i,u),a=o.circular,l=a||-1!==c.indexOf(o.value);l&&foundCircularReference(r,f,u);var t=$Ref.dereference(e,o.value);return l||(l=crawl(t,o.path,n,c,f,u)),l&&!a&&"ignore"===u.dereference.circular&&(t=e),a&&(t.$ref=n),{circular:l,value:t}}function foundCircularReference(e,r,n){if(r.circular=!0,!n.dereference.circular)throw ono.reference("Circular $ref pointer found at %s",e);return!0}var $Ref=require("./ref"),Pointer=require("./pointer"),ono=require("ono"),debug=require("./util/debug"),url=require("./util/url");module.exports=dereference;

},{"./pointer":10,"./ref":11,"./util/debug":16,"./util/url":19,"ono":69}],3:[function(require,module,exports){
(function (Buffer){
"use strict";function $RefParser(){this.schema=null,this.$refs=new $Refs}function normalizeArgs(e){var r=e[1],s=e[2];return"function"==typeof r&&(s=r,r=void 0),r instanceof Options||(r=new Options(r)),{schema:e[0],options:r,callback:s}}var Promise=require("./util/promise"),Options=require("./options"),$Refs=require("./refs"),parse=require("./parse"),resolve=require("./resolve"),bundle=require("./bundle"),dereference=require("./dereference"),url=require("./util/url"),maybe=require("call-me-maybe"),ono=require("ono");module.exports=$RefParser,module.exports.YAML=require("./util/yaml"),$RefParser.parse=function(e,r,s){var t=this;return(new t).parse(e,r,s)},$RefParser.prototype.parse=function(e,r,s){var t,n=normalizeArgs(arguments);if(!n.schema||"string"!=typeof n.schema&&"object"!=typeof n.schema){var a=ono("Expected a file path, URL, or object. Got %s",n.schema);return maybe(n.callback,Promise.reject(a))}if(this.schema=null,this.$refs=new $Refs,"object"==typeof n.schema)this.$refs._basePath="",this.$refs._add("",n.schema),t=Promise.resolve(n.schema);else{var o=n.schema;url.isFileSystemPath(o)&&(o=url.fromFileSystemPath(o)),o=url.resolve(url.cwd(),o),this.$refs._basePath=url.stripHash(o),t=parse(o,this.$refs,n.options)}var i=this;return t.then(function(e){if(!e||"object"!=typeof e||Buffer.isBuffer(e))throw ono.syntax('"%s" is not a valid JSON Schema',i.$refs._basePath||e);return i.schema=e,maybe(n.callback,Promise.resolve(i.schema))})["catch"](function(e){return maybe(n.callback,Promise.reject(e))})},$RefParser.resolve=function(e,r,s){var t=this;return(new t).resolve(e,r,s)},$RefParser.prototype.resolve=function(e,r,s){var t=this,n=normalizeArgs(arguments);return this.parse(n.schema,n.options).then(function(){return resolve(t,n.options)}).then(function(){return maybe(n.callback,Promise.resolve(t.$refs))})["catch"](function(e){return maybe(n.callback,Promise.reject(e))})},$RefParser.bundle=function(e,r,s){var t=this;return(new t).bundle(e,r,s)},$RefParser.prototype.bundle=function(e,r,s){var t=this,n=normalizeArgs(arguments);return this.resolve(n.schema,n.options).then(function(){return bundle(t,n.options),maybe(n.callback,Promise.resolve(t.schema))})["catch"](function(e){return maybe(n.callback,Promise.reject(e))})},$RefParser.dereference=function(e,r,s){var t=this;return(new t).dereference(e,r,s)},$RefParser.prototype.dereference=function(e,r,s){var t=this,n=normalizeArgs(arguments);return this.resolve(n.schema,n.options).then(function(){return dereference(t,n.options),maybe(n.callback,Promise.resolve(t.schema))})["catch"](function(e){return maybe(n.callback,Promise.reject(e))})};
...
(function (Buffer){
"use strict";var Promise=require("../util/promise"),BINARY_REGEXP=/\.(jpeg|jpg|gif|png|bmp|ico)$/i;module.exports={order:400,allowEmpty:!0,canParse:function(e){return Buffer.isBuffer(e.data)&&BINARY_REGEXP.test(e.url)},parse:function(e){return new Promise(function(r,t){r(Buffer.isBuffer(e.data)?e.data:new Buffer(e.data))})}};

}).call(this,require("buffer").Buffer)

},{"../util/promise":18,"buffer":24}],7:[function(require,module,exports){
(function (Buffer){
"use strict";var Promise=require("../util/promise");module.exports={order:100,allowEmpty:!0,canParse:/\.json$/i,parse:function(r){return new Promise(function(e,t){var i=r.data;Buffer.isBuffer(i)&&(i=i.toString()),e("string"==typeof i?0===i.trim().length?void 0:JSON.parse(i):i)})}};

}).call(this,{"isBuffer":require("../../node_modules/is-buffer/index.js")})

},{"../../node_modules/is-buffer/index.js":36,"../util/promise":18}],8:[function(require,module,exports){
(function (Buffer){
"use strict";var Promise=require("../util/promise"),TEXT_REGEXP=/\.(txt|htm|html|md|xml|js|min|map|css|scss|less|svg)$/i;module.exports={order:300,allowEmpty:!0,encoding:"utf8",canParse:function(t){return("string"==typeof t.data||Buffer.isBuffer(t.data))&&TEXT_REGEXP.test(t.url)},parse:function(t){var r=this;return new Promise(function(e,s){"string"==typeof t.data?e(t.data):Buffer.isBuffer(t.data)?e(t.data.toString(r.encoding)):s(new Error("data is not text"))})}};
...
"use strict";function $Refs(){this.circular=!1,this._basePath="",this._$refs={}}function getPaths(e,r){var t=Object.keys(e);return r=Array.isArray(r[0])?r[0]:Array.prototype.slice.call(r),r.length>0&&r[0]&&(t=t.filter(function(t){return-1!==r.indexOf(e[t].pathType)})),t.map(function(r){return{encoded:r,decoded:"file"===e[r].pathType?url.toFileSystemPath(r,!0):r}})}var ono=require("ono"),$Ref=require("./ref"),url=require("./util/url");module.exports=$Refs,$Refs.prototype.paths=function(e){var r=getPaths(this._$refs,arguments);return r.map(function(e){return e.decoded})},$Refs.prototype.values=function(e){var r=this._$refs,t=getPaths(r,arguments);return t.reduce(function(e,t){return e[t.decoded]=r[t.encoded].value,e},{})},$Refs.prototype.toJSON=$Refs.prototype.values,$Refs.prototype.exists=function(e,r){try{return this._resolve(e,r),!0}catch(t){return!1}},$Refs.prototype.get=function(e,r){return this._resolve(e,r).value},$Refs.prototype.set=function(e,r){e=url.resolve(this._basePath,e);var t=url.stripHash(e),s=this._$refs[t];if(!s)throw ono('Error resolving $ref pointer "%s". \n"%s" not found.',e,t);s.set(e,r)},$Refs.prototype._add=function(e,r){var t=url.stripHash(e),s=this._$refs[t]=new $Ref;return s.path=e,s.value=r,s.$refs=this,s},$Refs.prototype._resolve=function(e,r){e=url.resolve(this._basePath,e);var t=url.stripHash(e),s=this._$refs[t];if(!s)throw ono('Error resolving $ref pointer "%s". \n"%s" not found.',e,t);return s.resolve(e,r)},$Refs.prototype._get$Ref=function(e){e=url.resolve(this._basePath,e);var r=url.stripHash(e);return this._$refs[r]};

},{"./ref":11,"./util/url":19,"ono":69}],13:[function(require,module,exports){
"use strict";function resolve(e,r){if(!r.resolve.external)return Promise.resolve();try{debug("Resolving $ref pointers in %s",e.$refs._basePath);var s=crawl(e.schema,e.$refs._basePath+"#",e.$refs,r);return Promise.all(s)}catch(i){return Promise.reject(i)}}function crawl(e,r,s,i){var t=[];return e&&"object"==typeof e&&Object.keys(e).forEach(function(o){var u=Pointer.join(r,o),n=e[o];if($Ref.isExternal$Ref(n)){var l=resolve$Ref(n,u,s,i);t.push(l)}else t=t.concat(crawl(n,u,s,i))}),t}function resolve$Ref(e,r,s,i){debug('Resolving $ref pointer "%s" at %s',e.$ref,r);var t=url.resolve(r,e.$ref),o=url.stripHash(t);return e=s._get$Ref(o),e?Promise.resolve(e.value):parse(t,s,i).then(function(e){debug("Resolving $ref pointers in %s",o);var r=crawl(e,o+"#",s,i);return Promise.all(r)})}var Promise=require("./util/promise"),$Ref=require("./ref"),Pointer=require("./pointer"),parse=require("./parse"),debug=require("./util/debug"),url=require("./util/url");module.exports=resolve;
...
"use strict";function getResult(e,r,t,n){var u=e[r];return"function"==typeof u?u.apply(e,[t,n]):u instanceof RegExp?u.test(t.url):Array.isArray(u)?-1!==u.indexOf(t.extension):u}var Promise=require("./promise"),debug=require("./debug");exports.all=function(e){return Object.keys(e).filter(function(r){return"object"==typeof e[r]}).map(function(r){return e[r].name=r,e[r]})},exports.filter=function(e,r,t){return e.filter(function(e){return!!getResult(e,r,t)})},exports.sort=function(e){return e.forEach(function(e){e.order=e.order||Number.MAX_SAFE_INTEGER}),e.sort(function(e,r){return e.order-r.order})},exports.run=function(e,r,t){var n,u,o=0;return new Promise(function(i,f){function s(){if(n=e[o++],!n)return f(u);try{debug("  %s",n.name);var i=getResult(n,r,t,c);i&&"function"==typeof i.then?i.then(a,p):void 0!==i&&a(i)}catch(s){p(s)}}function c(e,r){e?p(e):a(r)}function a(e){debug("    success"),i({plugin:n,result:e})}function p(e){debug("    %s",e.message||e),u=e,s()}s()})};
